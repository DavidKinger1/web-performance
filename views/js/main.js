...
// declared var phase outside of loop
function updatePositions() {
    frame++;
    window.performance.mark("mark_start_frame");
    var movingPizzas = document.getElementById("movingPizzas1");
    var items = document.getElementsByClassName('mover');
    var numMovers = items.length;
    var numPhase = Math.abs(document.body.scrollTop / 1250);
    var phase;

    for (var i = 0; i < numMovers; i++) {
        phase = Math.sin(numPhase + (i % 5));
...
// replaced document.querySelectorAll with document.getElementById
// declared elem outside of loop also calculated number of pizzas to render

document.addEventListener('DOMContentLoaded', function() 
{
    var elem;
    var cols = window.innerHeight / 100;
    console.log("Columns are " + cols);
    var rows = window.innerWidth / 100;
    console.log("Rows are " + rows);
    var s = 256;
    var numPizzasToDraw = cols * rows;
  
  for (var i = 0; i < numPizzasToDraw; i++) {
